...
sim_name = "constant_climate4"
setwd("/home/sok10hm/Dokumente/ConstantSimuv4/")
# create and select
...
parameters <- list(
  ...
  eo = c(0.6), #rep(eo_gradient, each = length(c_intra_gradient)*length(f_gradient)),
  ...
  e_type = "constant", # "change", #environment type "change" or "constant"
...
write.table(output, "output.csv", col.names = FALSE, row.names= FALSE)
